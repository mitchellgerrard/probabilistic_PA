...
at each program location to update the symbolic variables.
...
All six traces through the control flow graph 
...
if the set of symbolic expressions at some location $l$ implies
...
A Markov chain is a labeled transition system which, given some
...
\ignore{$\sigma \in A^*$,} is termed a \textit{schedule} (or policy or 
