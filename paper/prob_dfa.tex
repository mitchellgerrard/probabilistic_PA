...
the abstract domain first on all execution environments which
satisfy the conditional, then on all environments which  
falsify the conditional; finally, we combine the two results.
If the satisfying execution environment is viewed as a set
of abstract states, then the falsifying environment
is defined as the complement of that set within the domain of all
possible execution environments.

The conditional filter is only applied to the first component of
...
$\{([-10,-6],0.1),([6,10],0.1)\}$; after applying the filter, 
we get $([6,10],0.1)$.
Combining these two results, our abstract domain after the
conditional becomes $\langle([-5,-1],0.1),([6,10],0.1)\rangle$.
