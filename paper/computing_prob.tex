...
The computation of $\sharp(\cdot)$ can be performed efficientyly for linear integer arithmetic (LIA) constraints.
...
\paragraph{Distribution-aware sampling.} The hit-or-miss Monte Carlo method we described offers a straightforward way to handle input distributions: the samples for the Binomial experiment can be simply drwan from the known distribution.
